...
const QRectF PublicConstants::SceneRect = QRectF(0, 0, 1000, 900);
 QVector2D PublicConstants::PlatformSize = QVector2D(80.0f, 10.0f);
...
// x - left, y - right bounds of platform center by x coordinate
 QVector2D PublicConstants::PlatformMoveBounds = QVector2D(PlatformSize.x() / 2, SceneRect.width() - PlatformSize.x() / 2);
